...
def make_dataset2(data_root: str,
                  data_list: str,
                  class_list: List[int]) -> List[Tuple[str, str, str]]:
    if not os.path.isfile(data_list):
        raise (RuntimeError("Image list file do not exist: " + data_list + "\n"))
    '''
        Recovers all tupples (img_path, label_path) relevant to the current experiments (class_list
        ...
        returns:
            image_label_list: List of (img_path, label_path) that contain at least 1 object of a class
                              in class_list
    '''
    image_label_list: List[Tuple[str, str, str]] = []
    list_read = open(data_list).readlines()

    print(f"Processing data for {class_list}")

    process_partial = partial(process_image2, data_root=data_root)

    for sublist in mmap_(process_partial, tqdm(list_read)):
        image_label_list += sublist

    return image_label_list


def process_image2(line: str,
                   data_root: str):
    '''
    ...
    saliency_name = os.path.join(data_root, line_split[2])
    item: Tuple[str, str, str] = (image_name, label_name, saliency_name)

    image_label_list = [item]
